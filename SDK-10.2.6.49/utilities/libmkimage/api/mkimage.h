...
/** DA1470x security configuration */
typedef struct {
        /** Private key - used in signature generation (32 bytes in length) */
        const uint8_t *priv_key;
        /** Symmetric key - used in executable encryption (32 bytes in length) */
         ...
         */
        const uint8_t *nonce;
} mkimage_security_data_da1470x_t;
...
/** DA1470x device administration configuration */
typedef struct {
        /** Number of keys which should be revoked */
        unsigned int key_rev_number;
        /** Array with keys which should be revoked */
...
} mkimage_device_adm_data_da1470x_t;
...
/**
 * \brief Create DA1470x device image
 *
 * Function creates image which could be used as SUOTA image for DA1470x devices. The image is created
 * from binary and version file data. If \p data is passed then image is created in secure mode -
 * executable (application binary) data is encrypted (using AES CTR) and signed (together with
 * device administration section). Ed25519 is used as algorithm of digital signature generation.
 * \p opt_data is an optional parameter and contains device administration configuration e.g.
 ...
 * \param [out] out                     allocated buffer with image data
 * \param [out] out_size                output buffer size
 * \param [in] offset                   image offset, used to determine if extra padding is needed
 *
 * \return command execution status
 *
 */
mkimage_status_t DLLEXPORT mkimage_create_da1470x_image(size_t in_size, const uint8_t *in,
                                                        size_t ver_size, const uint8_t *ver,
                                                        uint32_t fw_version,
