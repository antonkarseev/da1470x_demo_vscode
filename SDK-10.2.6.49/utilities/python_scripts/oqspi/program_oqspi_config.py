...
DEFAULT_CONFIG_PATH = os.path.join(os.path.join(os.path.dirname(__file__), 'program_oqspi.xml'))
DEFAULT_FLASH_CONFIGS = os.path.join(PROJECT_ROOT, "oqspi", "flash_configurations.xml")
...
class ProgramOQspiConfig(object):
    PROGRAM_OQSPI_TAG = 'program_oqspi'
    ...
    FLASH_CTRLMODE_REG_VALUE_TAG = 'flash_ctrlmode_reg_value'
    ...
    def __init__(self, config_path=None):
        ...
        self.flash_ctrlmode_reg_value = None
    ...
    def load(self):
        try:
            ...
            root = ElemTree.Element(self.PROGRAM_OQSPI_TAG)
        ...
        if product_header:
            ...
            flash_ctrlmode_reg_value = product_header.find(self.FLASH_CTRLMODE_REG_VALUE_TAG)
            self.flash_ctrlmode_reg_value = int(flash_ctrlmode_reg_value.text, 16) \
                if flash_ctrlmode_reg_value is not None else None
    ...
    def is_valid(self):
        ...
        if self.product_id == ProductId.DA1470x_00:
            return self.active_image_address is not None and self.update_image_address is not None \
                   ...
                   and self.flash_ctrlmode_reg_value is not None \
        ...
        for item in fconfig:
            try:
                if self.flash_burstcmda_reg_value == \
                    int(fconfig[item][ProgramOQspiConfig.FLASH_BURSTCMDA_REG_VALUE_TAG], 16) \
                    and self.flash_name == \
                    str(fconfig[item][ProgramOQspiConfig.FLASH_NAME_TAG]) \
                    and self.flash_size == \
                    int(fconfig[item][ProgramOQspiConfig.FLASH_SIZE_TAG], 16) \
                    and self.flash_burstcmdb_reg_value == \
                    int(fconfig[item][ProgramOQspiConfig.FLASH_BURSTCMDB_REG_VALUE_TAG], 16) \
                    and self.flash_ctrlmode_reg_value == \
                    int(fconfig[item][ProgramOQspiConfig.FLASH_CTRLMODE_REG_VALUE_TAG], 16) \
                    and self.flash_write_config_command == self.parse_config_command(
                        fconfig[item][ProgramOQspiConfig.FLASH_WRITE_CONFIG_COMMAND_TAG]):
                        return True
            except:
    ...
    def save(self):
        ...
        if self.product_id == ProductId.DA1470x_00:
            ...
            flash_ctrlmode_reg_value = ElemTree.Element(self.FLASH_CTRLMODE_REG_VALUE_TAG)
            flash_ctrlmode_reg_value.text = hex(self.flash_ctrlmode_reg_value)
            product_header.append(flash_ctrlmode_reg_value)
    ...
    def get_config_info(self):
        ...
        if self.product_id == ProductId.DA1470x_00 and self.product_id is not None:
            buf += 'Product Header:\n'
            ...
            buf += '\tflash_ctrlmode_reg_value: {}\n'.format(
                hex(self.flash_ctrlmode_reg_value) if self.flash_ctrlmode_reg_value is not None
                else 'N/A')
...
def _setup_address(value_name, ui_message):
    ...
    while True:
        user_address = ui.ask_value(value_name=value_name, text=ui_message, default='0x3000')
...
def program_oqspi_config(product_id=None, flash_config_path=None, header_config_path=None,
                        flash_configuration=None, default_img_addr=False):
    config = ProgramOQspiConfig(header_config_path)
    ...
    if config.product_id == ProductId.DA1470x_00:
        ...
        config.flash_size = int(configuration[ProgramOQspiConfig.FLASH_SIZE_TAG], 16)
        config.flash_burstcmda_reg_value = \
            int(configuration[ProgramOQspiConfig.FLASH_BURSTCMDA_REG_VALUE_TAG], 16)
        config.flash_burstcmdb_reg_value = \
            int(configuration[ProgramOQspiConfig.FLASH_BURSTCMDB_REG_VALUE_TAG], 16)
        config.flash_ctrlmode_reg_value = \
            int(configuration[ProgramOQspiConfig.FLASH_CTRLMODE_REG_VALUE_TAG], 16)
        config.flash_write_config_command = config.parse_config_command(
            configuration[ProgramOQspiConfig.FLASH_WRITE_CONFIG_COMMAND_TAG])

        if default_img_addr:
            config.active_image_address = 0x3000
            config.update_image_address = 0x3000
        else:
            ui.print_title('Active FW Image Address\n')
            config.active_image_address = \
                _setup_address(ProgramOQspiConfig.ACTIVE_FW_IMAGE_ADDRESS_TAG,
                               'Insert Active FW image address (hex)')

            ui.print_title('Update FW Image Address\n')
            config.update_image_address = \
                _setup_address(ProgramOQspiConfig.UPDATE_FW_IMAGE_ADDRESS_TAG,
...
if __name__ == '__main__':
    ui.print_header('PROGRAM OQSPI CONFIGURATOR')
    run_script(program_oqspi_config, parse_args)
