...
from oqspi.program_oqspi_config import ProductId
from oqspi.program_oqspi import MIN_FW_ADDRESS, make_product_header, handle_oqspi_config
...
def prepare_flash_image(bin_file, image_file=None, sw_version_file=None, header_config_path=None,
    ...
    if oqspi_config.update_image_address < MIN_FW_ADDRESS:
        raise RuntimeError("FW cannot be placed at address lower than 0x{:X} (0x{:X} "
                           "given)".format(MIN_FW_ADDRESS, oqspi_config.update_image_address))
    ...
    product_header = make_product_header(oqspi_config.active_image_address,
                                         oqspi_config.update_image_address,
                                         oqspi_config.flash_burstcmda_reg_value,
                                         oqspi_config.flash_burstcmdb_reg_value,
                                         oqspi_config.flash_ctrlmode_reg_value,
                                         oqspi_config.flash_write_config_command)
    image_data = product_header * 2
    padding_size = oqspi_config.update_image_address - len(image_data)
    ...
    try:
        ...
        if sw_version_file is None:
            try:
                ...
                Mkimage().da1470x(bin_file, version_file, app_mkimage_file, 0)
            finally:
                os.remove(version_file)
        else:
            Mkimage().da1470x(bin_file, sw_version_file, app_mkimage_file, 0)
