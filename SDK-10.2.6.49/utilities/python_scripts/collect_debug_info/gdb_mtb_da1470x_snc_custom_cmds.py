...
MTB_BASE                =  0xF0002000   # DA1470x-SNC MTB BASE address value

MTB_POSITION_REG = {
    MTB_REGISTER_ADDRESS: MTB_BASE,
    MTB_REGISTER_FIELDS: {
        'WRAP': 0x00000004,
        'POINTER': 0xFFFFFFF8
    }
...
MTB_MASTER_REG = {
    MTB_REGISTER_ADDRESS: MTB_BASE + 0x4,
    MTB_REGISTER_FIELDS: {
        'MASK': 0x0000001F,
        'TSTARTEN': 0x00000020,
        'TSTOPEN': 0x00000040,
...
MTB_FLOW_REG = {
    MTB_REGISTER_ADDRESS: MTB_BASE + 0x8,
    MTB_REGISTER_FIELDS: {
        'AUTOSTOP': 0x00000001,
        'AUTOHALT': 0x00000002,
...
MTB_BASE_REG = {
    MTB_REGISTER_ADDRESS: MTB_BASE + 0xc,
    MTB_REGISTER_FIELDS: {
        'BASE': 0xFFFFFFFF,
    }
}
...
def is_mtb_en():
    if read_word(MTB_MASTER_REG[MTB_REGISTER_ADDRESS]) & MTB_MASTER_REG[MTB_REGISTER_FIELDS]['EN']:
        return True
    else:
        return False

class MTBReadRegistersDA1470xSNC(gdb.Command):
    def __init__(self):
        super(MTBReadRegistersDA1470xSNC, self).__init__("mtb-reg-DA1470x-snc", gdb.COMMAND_STATUS, gdb.COMPLETE_NONE)
...
MTBReadRegistersDA1470xSNC()


class MTBDownloaderDA1470xSNC(gdb.Command):
    def __init__(self):
        super(MTBDownloaderDA1470xSNC, self).__init__("mtb-fetch-DA1470x-snc", gdb.COMMAND_STATUS, gdb.COMPLETE_NONE)
