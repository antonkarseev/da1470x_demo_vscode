...
int crypto_rng_bytes(void *ud, unsigned char *buff, size_t len)
{
...
bool crypto_symmetric_key_gen(crypto_buffer_t *key)
{
        ...
        crypto_rng_bytes(NULL, key->value, key->size);
                                                                        ...
                                                                        crypto_buffer_t *pub_key)
{
        ...
        if (elliptic_curve == ELLIPTIC_CURVE_EDWARDS25519) {
                ...
                crypto_rng_bytes(NULL, priv_key->value, priv_key->size);
        ...
        for (i = 0; i < GENERATION_TRY_NUM; i++) {
                if (mbedtls_ecp_gen_key(ec_info.mbedtls_id, &keypair, crypto_rng_bytes, NULL)) {
                        goto cleanup;
                }
                                                                        ...
                                                                        crypto_buffer_t *sig)
{
        ...
        if (mbedtls_ecdsa_sign(&ecp_group, &r, &s, &d, sha, sha_len, crypto_rng_bytes, NULL)) {
                goto cleanup;
        }
