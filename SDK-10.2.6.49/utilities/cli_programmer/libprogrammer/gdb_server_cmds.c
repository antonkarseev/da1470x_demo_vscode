...
static int get_gdb_server_pid(const char *gdb_cmdline, int port)
{
        ...
        char proc_name[300]; /* expected gdbserver name taken from gdb_cmdline */

        char proc_name_with_path[256];
        int ret;
        FILE *fp;
        ...
        snprintf(cmd, sizeof(cmd), "lsof -nPi -sTCP:LISTEN 2>/dev/null | awk '/:%d / { print $2 }'",
                                                                                port);
#endif
        ...
        ret = fscanf(fp, "%s", proc_name_with_path);
        pclose(fp);

        get_name_from_path(proc_name_with_path, proc_name);

        if (ret < 1 || strstr(proc_name_with_path, gdb_name) == NULL) {
                prog_print_log("proc_name invalid\n");
                return PID_INVALID;
        }
...
/* return the RAM address that is remapped to 0 */
static int get_boot_addr(uint32_t *boot_addr)
{
        const prog_chip_regs_t *regs;
        const char *chip_rev;
        int status = 0;
        ...
        if ((strncmp(chip_rev, CHIP_REV_700AB, CHIP_REV_STRLEN) == 0)
                        && ((regs->sys_ctrl_reg_val & 7) == 5)) {
                /* using remap of RAM3 to 0 */
                *boot_addr = 0x20010000;
        } else {
                *boot_addr = 0;
        }
...
static int gdb_server_upload_executable(uint8_t *executable_code, size_t executable_code_size)
{
        ...
        /* get the address to copy the executable to */
        if ((status = get_boot_addr(&boot_addr) != 0)) {
                return status;
        }

        /* write boot loader code from array to platform */
        if ((status = gdb_server_loadbin(executable_code, executable_code_size, boot_addr)) != 0) {
                return status;
        }
...
static bool gdb_server_cmd_dummy(void)
{
        ...
        header_buf[2] = (uint8_t) 0;
        header_buf[3] = (uint8_t) (0 >> 8);
...
static void gdb_server_stop(int pid)
{
        ...
        /* GDB server was started by CLI wait for graceful end first */
        ...
        /* Stop GDB server forcefully */
        sprintf(kill_cmd, "%%WINDIR%%\\system32\\TASKKILL /F /PID %d >nul 2>nul", (int) pid);
        system(kill_cmd);
...
int gdb_server_cmd_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        if (!strncmp(chip_rev, CHIP_REV_690AB, CHIP_REV_STRLEN) ||
            !strncmp(chip_rev, CHIP_REV_700AB, CHIP_REV_STRLEN)) {
                cell_32bit = true;
        }
...
int gdb_server_cmd_mass_erase_eflash(void)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_MASS_ERASE_EFLASH;
        header_buf[2] = (uint8_t) 0;
        header_buf[3] = (uint8_t) (0 >> 8);
...
int gdb_server_cmd_direct_write_to_oqspi(const uint8_t *buf, size_t size, uint32_t addr, bool verify)
{
        ...
        /* create constant part of header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_DIRECT_WRITE_TO_OQSPI;
...
int gdb_server_cmd_chip_erase_oqspi(uint32_t address)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_CHIP_ERASE_OQSPI;
        header_buf[2] = (uint8_t) 4;
        header_buf[3] = (uint8_t) (4 >> 8);
...
int gdb_server_cmd_is_empty_oqspi(unsigned int size, unsigned int start_address, int *ret_number)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_IS_EMPTY_OQSPI;
        header_buf[2] = (uint8_t) 8;
        header_buf[3] = (uint8_t) (8 >> 8);
        header_buf[4] = (uint8_t) (size);
        header_buf[5] = (uint8_t) (size >> 8);
...
int gdb_server_cmd_get_oqspi_state(flash_dev_info_t *oqspi_flash_info)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_GET_OQSPI_STATE;
        header_buf[2] = (uint8_t) 0;
        header_buf[3] = (uint8_t) (0 >> 8);
        ...
        return (gdb_server_frame_to_uint8_buf((uint8_t *)oqspi_flash_info, sizeof(flash_dev_info_t)) ? 0 :
                                                                ERR_GDB_SERVER_INVALID_RESPONSE);
}
