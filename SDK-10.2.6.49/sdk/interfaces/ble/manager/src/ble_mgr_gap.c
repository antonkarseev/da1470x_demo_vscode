...
static void gap_skip_latency_complete(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_skip_latency_rsp_t *rsp = ble_msg_init(BLE_MGR_GAP_SKIP_LATENCY_CMD, sizeof(*rsp));
        if (gtl != NULL) {
                struct gapc_cmp_evt *gevt = (void *) gtl->param;
                switch (gevt->status) {
                case GAP_ERR_NO_ERROR:
...
void ble_mgr_gap_skip_latency_cmd_handler(void *param)
{
        ...
        if (!dev) {
                ...
                ble_mgr_common_stack_msg_t *gmsg;
                struct gapc_set_skip_latency_cmd *gcmd;
                /* Setup GTL message */
                gmsg = ble_gtl_alloc_with_conn(GAPC_SET_SKIP_LATENCY_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
                gcmd = (struct gapc_set_skip_latency_cmd *) gmsg->msg.gtl.param;
                gcmd->operation = GAPC_SET_SKIP_LATENCY;
                gcmd->enable = enable;

                // Block (no response) until complete message received from controller
                ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_SET_SKIP_LATENCY,
                                      gap_skip_latency_complete, (void *) NULL);
                ble_gtl_send(gmsg);
