...
static uint16_t get_tx_length(sp_service_t *sps, uint16_t conn_idx)
{
        ...
        ble_storage_get_u32(conn_idx, sps->sps_tx_val_h, &length);
...
static int calculate_length(uint16_t conn_idx, int length)
{
    uint16_t mtu;
    char    *pdu;
...
void sps_tx_data(ble_service_t *svc, uint16_t conn_idx, uint8_t *data, uint16_t length)
{
        ...
        length = calculate_length(conn_idx, length);
