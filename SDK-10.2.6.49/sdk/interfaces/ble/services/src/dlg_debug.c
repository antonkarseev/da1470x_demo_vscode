...
int dlgdebug_notify_vprintf(ble_service_t *svc, uint16_t conn_idx, const char *fmt, va_list argp)
{
        dlgdebug_service_t *dbgs = (dlgdebug_service_t *) svc;
        uint16_t mtu;
        char *pdu;
        ...
        int len = vsnprintf(pdu, mtu - 3, fmt, argp);

        ble_gatts_send_event(conn_idx, dbgs->cp_val_h, GATT_EVENT_NOTIFICATION, len, pdu);
