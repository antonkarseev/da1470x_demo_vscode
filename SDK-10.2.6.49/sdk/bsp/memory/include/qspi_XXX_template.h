...
static const qspi_flash_config_t flash_XXX_config = {
        ...
        .qpi_mode                         = false,
        .is_ram                           = false,
        .memory_size                      = MEMORY_SIZE_XXXMb,
};

__STATIC_FORCEINLINE __UNUSED uint8_t flash_XXX_read_custom_config_register(void)
{
...
__STATIC_FORCEINLINE __UNUSED void flash_XXX_write_custom_config_register(uint8_t value)
{
        __DBG_QSPI_VOLATILE__ uint8_t status;
...
__RETAINED_CODE static void flash_XXX_sys_clock_cfg(sys_clk_t sys_clk)
{
        ...
        if (sys_clk == sysclk_PLL96) {
                hw_qspi_set_div(HW_QSPIC, HW_QSPI_DIV_2);
        }
        else {
